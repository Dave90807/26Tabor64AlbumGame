using _26Tabor64AlbumGame.Models;
using System;
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private async Task<List<StorageFile>> PickRandomSongs(ObservableCollection<StorageFile> allSongs)
        {
            ...
            while (randomSongs.Count < 10)
            {
                ...
                // Find random songs UBT:
                // 1) Don't pick the same song twice!
                // 2) Don't pick a song from an album that I've already pciked.
        ...
        {
            // Ignor clicks when we are in cooldown
        ...
        private async Task<ObservableCollection<StorageFile>> SetupMusicList()
        {
            ...
            string path = "C:\\Users\\DaveC\\Music\\BobsFakeMusicLibrary";
            StorageFolder bobsMusicFolder = await StorageFolder.GetFolderFromPathAsync(path);
            var allSongs = new ObservableCollection<StorageFile>();
            await RetrieveFilesInFolders(allSongs, bobsMusicFolder);  // replaced "folder" w "bobsMusicFolder"
            return allSongs;
        }
